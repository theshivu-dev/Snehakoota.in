...
(function(){
  ...
  const state = { updates: [], fullBoardOpen: false };
  ...
  function renderFullItem(update) {
    ...
    const location = update.event && update.event.locationName ? '<div class="sk-notice-event-location">📍 ' + escapeHtml(update.event.locationName) + '</div>' : "";
    const eventInfo = eventDate ? '<div class="sk-notice-event-date">📅 ' + escapeHtml(eventDate) + '</div>' : "";
      ...
      '<article class="sk-notice-full-item" data-notice-id="', update.id, '">',
        ...
        '<div class="sk-notice-full-detail" hidden>',
          '<div class="sk-notice-full-actions" data-notice-actions hidden>',
            '<button class="sk-notice-delete" type="button" data-notice-delete="', update.id, '" aria-label="Delete ', escapeHtml(update.title), '"><span aria-hidden="true">⌫</span><span>Delete notice</span></button>',
  ...
  function render() {
    ...
    if (elements.previewStatus) elements.previewStatus.textContent = state.updates.length === 1 ? "1 current update" : state.updates.length + " current updates";
    if (elements.fullStatus) elements.fullStatus.textContent = "";
    bindItemActions();
  ...
  function openFullBoard(focusId) {
    ...
    window.requestAnimationFrame(function() {
      ...
      } else if (elements.closeButton) elements.closeButton.focus({ preventScroll: true });
    });
  }
  ...
  function closeFullBoard() {
    ...
    window.setTimeout(function() { if (!state.fullBoardOpen) elements.fullBoard.hidden = true; }, 220);
  }
  ...
  async function loadManageCapabilities() {
    ...
    try {
      ...
      elements.fullList.querySelectorAll('[data-notice-actions]').forEach(function(actions) {
        ...
        if (!item) return;
        actions.hidden = capabilities.get(Number(item.getAttribute('data-notice-id'))) !== true;
      });
    } catch (error) {
  ...
  async function refreshNotices() {
    const elements = getElements();
    if (!window.SnehakootaNoticesService) { console.error("Notice service is unavailable."); return; }
  ...
  function init() { bindBoardActions(); refreshNotices(); }
  if (document.readyState === "loading") document.addEventListener("DOMContentLoaded", init, { once: true }); else init();

  window.SnehakootaNoticeBoard = Object.freeze({ refresh: refreshNotices, open: openFullBoard, close: closeFullBoard, HOME_NOTICE_LIMIT: HOME_NOTICE_LIMIT });
})();