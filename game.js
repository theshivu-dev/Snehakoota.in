/* ===================================================================
   SNEHAKOOTA GAME WIDGET — JavaScript
   -------------------------------------------------------------------
   Self-mounting version extracted from the original index.html game
   <script>...</script> block.

   The host page now only needs:
       <div id="sk-game-root"></div>
       <script src="game.js"></script>

   This file creates the launcher, confirmation bubble and game panel
   inside #sk-game-root, registers the available game modules, and
   wires their controls.

   No game-fix.js or game-repair.js code is included here; those remain
   separate historical repair files until independently verified safe
   to retire.
   =================================================================== */
(function(){
  "use strict";

  var mount = document.getElementById('sk-game-root');
  if (!mount) return;

  /* ---------------------------------------------------------------
     GAME SHELL MARKUP
     The former inline game HTML is created here so host pages only
     need the single #sk-game-root mount element.
     --------------------------------------------------------------- */
  mount.innerHTML =
    '<button class="skg-launcher skg-pulsing" id="skgLauncher" aria-label="Play a quick game">'
      + '<svg viewBox="0 0 24 24" fill="none" stroke="#FFF6E8" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">'
        + '<rect x="2" y="7" width="20" height="11" rx="4"></rect>'
        + '<path d="M7 11v3M5.5 12.5h3"></path>'
        + '<circle cx="16" cy="11.5" r="1"></circle>'
        + '<circle cx="18.5" cy="14" r="1"></circle>'
      + '</svg>'
    + '</button>'
    + '<span class="skg-hint">ಸ್ವಲ್ಪ ಆಟ ಆಡೋಣವಾ?</span>'
    + '<div class="skg-veil" id="skgVeil"></div>'
    + '<div class="skg-confirm" id="skgConfirm">'
      + '<p>ಸ್ವಲ್ಪ ಆಟ ಆಡೋಣವಾ?</p>'
      + '<div class="skg-row">'
        + '<button class="skg-no" id="skgNo">ಬೇಡ</button>'
        + '<button class="skg-yes" id="skgYes">ಹೌದು ▶</button>'
      + '</div>'
    + '</div>'
    + '<div class="skg-panel" id="skgPanel">'
      + '<div class="skg-panel-top">'
        + '<span class="skg-title" id="skgPanelTitle">⏱ ಸರಿಯಾದ ಸಮಯಕ್ಕೆ ಒತ್ತಿ</span>'
        + '<button class="skg-panel-close" id="skgClose" aria-label="Close game">&#10005;</button>'
      + '</div>'
      + '<div class="skg-game-area" id="skgGameArea"></div>'
    + '</div>';
  ...
  /* ===================================================================
     ...
     window.SKG.games so the whole widget remains portable.
     =================================================================== */
