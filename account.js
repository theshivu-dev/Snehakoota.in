...
(function(){
  ...
  supabaseClient.auth.getSession().then(function(res){if(res.error){console.error('Initial session error:',res.error);lastAuthState=false;updateAccount(null);return;}var session=res.data.session;lastAuthState=!!(session&&session.user);updateAccount(session);});
})();
